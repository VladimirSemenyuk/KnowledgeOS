...
- `Posts/content-index.md`, если файл существует;
- существующие файлы в `Posts/published` открывать выборочно после поиска по `linkedin_activity_id`, `linkedin_url`, дате или ключевой фразе.
...
Перед созданием файла проверить `Posts/content-index.md` и `Posts/published` через `rg` по:
...
- обновить `Posts/content-index.md`, если он существует.
