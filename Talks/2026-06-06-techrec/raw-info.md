...
Дополнительный источник: [[dora-roi-of-ai-assisted-software-development-2026]].
...
DORA усиливает этот вывод: AI является amplifier of the system. Он не заменяет engineering operating model, а увеличивает эффект уже существующей системы. Зрелая система получает больше throughput и больше экспериментов. Незрелая система получает больше непроверенной сложности, review bottlenecks, security-риск и delivery instability.
...
DORA-добавление: ROI не должен считаться как headcount reduction. Более управленчески зрелая логика — считать высвобожденную capacity и реинвестировать ее в продуктовые эксперименты, устранение технического долга, reliability и faster value delivery.
...
DORA-добавление: это и есть ответ на verification tax. AI снижает стоимость генерации кода, но не снижает автоматически стоимость понимания, проверки, security и эксплуатации. Если verification не встроена в pipeline, AI ускоряет производство непроверенной сложности.
...
DORA-добавление: AI-accessible internal data — одно из условий ROI. Внутренний контекст должен быть актуальным, структурированным и пригодным для использования агентами. Иначе AI работает на внешних вероятностях, а не на реальном operating model компании.
...
DORA-добавление: DORA metrics дают язык для связи engineering transformation и financial ROI.

| Метрика | Управленческий смысл |
|---|---|
| Lead time for changes | скорость превращения engineering work в value |
| Deployment frequency | throughput системы и частота продуктовых экспериментов |
| Change failure rate | стоимость нестабильности и rework |
| Failed deployment recovery time / MTTR | стоимость downtime и операционного риска |

Особенно полезный leading indicator — experiment frequency. AI снижает стоимость продуктового эксперимента, но ценность появляется только если delivery system умеет безопасно проверять больше гипотез.
...
- AI усиливает слабую систему: растет объем изменений, но review, testing, security и deployment остаются прежними.
...
- J-Curve воспринимается как провал, хотя первые месяцы adoption могут включать нормальную просадку продуктивности из-за learning curve, verification tax и pipeline adaptation.
...
10. ROI от AI нужно считать через bottlenecks, experiment frequency, rework, downtime и реинвестированную capacity, а не через сокращение команды.
